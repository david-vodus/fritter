...
class ProfilesTweets extends StatefulWidget {
  ...
  const ProfilesTweets({Key? key, required this.user, required this.type, required this.includeReplies, required this.pinnedTweets})
      : super(key: key);
...
class _ProfilesTweetsState extends State<ProfilesTweets> with AutomaticKeepAliveClientMixin<ProfilesTweets> {
  ...
  Future _loadTweets(String? cursor) async {
    try {
      ...
      var result2 = await Twitter.getTweets(widget.user.idStr!, widget.type, widget.pinnedTweets,
          cursor: cursor, count: pageSize, includeReplies: widget.includeReplies);
      List<TweetChain> chainALL=[];
      chainALL.addAll(result.chains);
      chainALL.addAll(result2.chains);
      TweetStatus statusAll=TweetStatus(chains: chainALL, cursorBottom: result.cursorBottom, cursorTop: result2.cursorTop);
      if (!mounted) {
        return;
      }
